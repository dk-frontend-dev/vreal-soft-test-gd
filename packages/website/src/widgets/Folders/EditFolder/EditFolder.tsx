...
function EditFolder({isOpen, onClose, closeDialog, selectedFolder}: EditFolderProps) {
  ...
  return (
    <Dialog open={isOpen} onClose={onClose}>
      <DialogTitle id="alert-dialog-title">Edit Folder</DialogTitle>
      <DialogContent>
        <form onSubmit={handleSubmit(updateFolder)} id={'update-folder'}>
          ...
          <Controller
            ...
            render={({field, fieldState: {error}}) => (
              <>
                <TextField {...field} type="text" fullWidth label={'Folder name'} error={error !== undefined} helperText={error ? error.message : ''} />
              </>
            )}
          ></Controller>
          ...
          <Controller
            ...
            render={({field}) => <AppMultiSelect sx={{width: '100%'}} valueKey={'email'} field={field} label={'Granted users'} options={allUsers} />}
          ></Controller>
