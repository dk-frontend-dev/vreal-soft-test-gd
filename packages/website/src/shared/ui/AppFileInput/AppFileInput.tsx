...
function AppFileInput({onDrop, fieldState, onChange, onBlur, filename, acceptedFileTypes = defaultFileTypes}: AppFileInputProps) {
  return (
    <Dropzone noClick onDrop={onDrop} accept={acceptedFileTypes}>
      {({getRootProps, getInputProps, open, isDragActive}) => (
        <div>
          <div className={clsx(s.container, {[s.active]: isDragActive})} {...getRootProps()}>
            <input
              {...getInputProps({
                id: 'spreadsheet',
                onChange,
            ...
            <div>{fieldState.error && <span role="alert">{fieldState.error.message}</span>}</div>
          </div>
        </div>
      )}
    </Dropzone>
